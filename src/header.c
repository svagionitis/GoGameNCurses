...
void print_header(WINDOW *win)
{
    ...
    memset(buf, '\0', sizeof buf);
    char_ret[i] = snprintf(buf, sizeof buf, "Main Window Max Height: %d", win_set.maxMainWindowHeight);
    color_str(win, 0, ++header_width, 0, 0, buf);
    header_width += char_ret[i++];

    memset(buf, '\0', sizeof buf);
    char_ret[i] = snprintf(buf, sizeof buf, "Main Window Max Width: %d", win_set.maxMainWindowWidth);
    color_str(win, 0, ++header_width, 0, 0, buf);
    header_width += char_ret[i++];
